...
router.post('/login',function(req,res,next){
    ...
    conn.query('select * from t_user where login_id=\''+id+'\' and password=\''+pwd+'\''
...
router.post('/join',function(req,res,next){
    ...
    if (username && userPwd && userPwdChk) {
        ...
        conn.query('SELECT * FROM t_user WHERE name = ?', [username], function(error, results, fields) { // DB에 같은 이름의 회원아이디가 있는지 확인
            if (error) throw error;
            if (results.length==0 && userPwd == userPwdChk) {     // DB에 같은 이름의 회원아이디가 없고, 비밀번호가 올바르게 입력된 경우 
                conn.query('INSERT INTO t_user (login_id,password,name,nickname,email,phone) VALUES(?,?,?,?,?,?)', [userid, userPwd,username,userid,email,phone], function (error, data) {
